...
function Product() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoader(true);
    try {
      const { data } = await axios.post(`${import.meta.env.VITE_API}/products/${id}/review`,
        myReview
        , {
          headers: {
    ...
    <>
      <div className={`container  `}>
        <div className={`d-flex  flex-wrap row-gap-3 justify-content-between ${style.box} position-relative`}>
          ...
          <div className={`col-lg-6 col-12 d-flex flex-sm-row flex-column justify-content-between gap-md-3 gap-2`}>
            <div className={`${style.mainImg}  d-flex flex-column justify-content-md-start  justify-content-center`}>
              <img src={displayImg} />
            </div>
            ...
            <div className={` ${style.imgContainer} d-flex flex-sm-column justify-content-start gap-md-3 gap-2 `}>
              ...
              </button>
              {
                ...
                product.subImages.map((category, index) =>
                  <button key={index} onClick={() => setDisplayImg(category.secure_url)} className={`${style.imgButton}`}>
        ...
        <div className={` ${style.reviewBox} d-flex flex-column gap-3`}>
          ...
          {loggedIn ?
            <form onSubmit={handleSubmit} className={`${style.review} p-sm-4 p-3 d-flex gap-3 flex-wrap  position-relative top-0 `}>
              <div className={`d-flex gap-3 `}>
                <img className={`${style.reviewImg}`} src={User.image.secure_url} />
                <div className={`d-flex flex-column gap-2`}>
                  <span className={`fs-5 CrimsonTextFont color1 fw-semibold`}>{User.userName}</span>
              ...
              <input type='text' placeholder='Add comment...' name='comment' onChange={handleChange} className={`align-self-stretch m-0 CrimsonTextFont ${style.comment} p-3 border w-100 `} />
              <input type='submit' value='Add' className={` px-5 bgcolor1 whiteC border-0 py-1 rounded CrimsonTextFont`}></input>
            </form>
            : <></>
          }
          {
            product.reviews.map((review) =>
              <div className={`${style.review} p-sm-4 p-3 d-flex gap-3 flex-wrap  position-relative top-0 `}>
                <div className={`d-flex gap-3 `}>
                  <img className={`${style.reviewImg}`} src={review.createdBy.image.secure_url} />
                  <div className={`d-flex flex-column gap-2`}>
                    <span className={`fs-5 CrimsonTextFont color1 fw-semibold`}>{review.createdBy.userName}</span>
                    <div className={` d-flex gap-2`}>
                ...
                <p className={`align-self-stretch m-0 CrimsonTextFont ${style.comment} p-3 border w-100 `}>{review.comment}</p>
